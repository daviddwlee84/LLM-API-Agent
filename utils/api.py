...
class OpenAPIManager:
    ...
    _spec: dict | list | ReducedOpenAPISpec
    _spec_path: str

    def __init__(self, yaml_url_or_path: str | None = None, default_model: str = 'gpt-4', reduce_spec: bool = True):
        self.encoder = tiktoken.encoding_for_model(default_model)
        self._to_reduce_spec = reduce_spec
    ...
    def spec_endpoints(self) -> int:
        if not self._to_reduce_spec:
            endpoints = [
                (route, operation)
                for route, operations in self._spec["paths"].items()
                for operation in operations
                if operation in ["get", "post"]
            ]
        else:
            endpoints = self._spec.endpoints
        return len(endpoints)

    def load_online_yaml(self, yaml_url: str, update_spec: bool = True) -> dict | list | ReducedOpenAPISpec:
        """
        ...
        api_spec = yaml.safe_load(response.content.decode('utf-8'))

        if self._to_reduce_spec:
            api_spec = reduce_openapi_spec(api_spec)
    ...
    def load_local_yaml(self, yaml_path: str, update_spec: bool = True) -> dict | list | ReducedOpenAPISpec:
        with open(yaml_path) as f:
            api_spec = yaml.load(f, Loader=yaml.Loader)
...
if __name__ == '__main__':
    manager = OpenAPIManager("https://raw.githubusercontent.com/APIs-guru/openapi-directory/main/APIs/spotify.com/1.0.0/openapi.yaml")
    print(manager._spec)
